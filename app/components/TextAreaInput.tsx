      ...
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        <g clipPath="url(#clip0_helpIcon)">
          <path
            ...
            stroke={destructive ? colors['text-destructive'] : colors['text-secondary']}
            strokeWidth="1.33333"
            strokeLinecap="round"
...
const TextAreaInput: React.FC<TextAreaInputProps> = ({
  ...
  const baseStyles = {
    backgroundColor: colors[getInputToken('normal', 'bg', destructive, disabled)],
    color: colors[getInputToken('normal', 'fg', destructive, disabled)],
    borderColor: colors[getInputToken('normal', 'border', destructive, disabled)],
  };

  const hoverStyles = {
    backgroundColor: colors[getInputToken('hover', 'bg', destructive, disabled)],
    color: colors[getInputToken('hover', 'fg', destructive, disabled)],
    borderColor: colors[getInputToken('hover', 'border', destructive, disabled)],
  };

  const focusStyles = {
    backgroundColor: colors[getInputToken('focus', 'bg', destructive, disabled)],
    color: colors[getInputToken('focus', 'fg', destructive, disabled)],
    borderColor: colors[getInputToken('focus', 'border', destructive, disabled)],
  };
    ...
    <div className="flex flex-col gap-1.5 w-full">
      {label && (
        <div className="flex gap-2 items-center">
          ...
          {required && (
            <span 
              style={{ color: destructive ? colors['text-destructive'] : colors['text-primary'] }}
              className="text-sm font-medium"
            >
              *
            </span>
      ...
      >
        ...
        <div className="absolute top-3 right-3">
          {destructive 
            ...
            : helpIcon && <HelpIcon tooltipText={tooltipText} destructive={destructive} />
          }
        </div>
      </div>
