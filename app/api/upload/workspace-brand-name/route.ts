...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (existingWorkspace?.brand_name_img_url) {
      ...
      if (deleteError) {
        console.error('Failed to delete existing brand name image:', deleteError);
      }
    }
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (!workspace?.brand_name_img_url) {
      return NextResponse.json({ error: 'No brand name image found' }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: 'Failed to delete brand name image' }, { status: 500 });
  }
}
