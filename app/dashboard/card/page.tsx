...
export default function Home() {
    ...
    const menuRef = useRef(null); // Reference to track the menu
    ...
    return (
      <div className="relative flex min-h-screen flex-col items-center justify-center bg-gray-100 p-6">
        <div 
          className="relative h-[164.75px] px-6 py-5 bg-white rounded-[10px] border border-[#e4e7ec] flex-col justify-start items-start gap-4 inline-flex overflow-visible"
        ...
        >
          ...
          {isMenuOpen && (
            ...
            >
              ...
              {[
                ...
                item === "separator" ? (
                  <div key={`sep-${index}`} className="w-full border-t border-[#e4e7ec]" />
                ) : (
                  <div key={index} className="px-4 py-3 flex items-center gap-2 transition duration-300 hover:bg-[#F9FAFB] cursor-pointer">
                    {item.icon && (
                      <img 
                    ...
                    <span className="text-[#344054] text-sm font-medium font-['Inter'] leading-tight">{item.label}</span>
                  </div>
                )
              )}
            </div>
          ...
          <div className="flex-col justify-start items-start gap-2 flex">
            ...
            <div className="text-[#101828] text-sm font-semibold font-['Inter'] leading-tight">Create a new task in Clandestine</div>
          </div>
          <div className="flex-col justify-start items-start gap-2 flex">
            <div className="self-stretch justify-start items-start gap-2 inline-flex">
              <div className="pl-1 pr-1.5 py-0.5 bg-[#eef3ff] rounded-md border-[#c6d7fe] justify-start items-center gap-1 flex">
                <div className="text-center text-[#3537cc] text-xs font-medium font-['Inter'] leading-[18px]">Human Resources</div>
              </div>
              <div className="pl-1 pr-1.5 py-0.5 bg-[#eff8ff] rounded-md border-[#b2ddff] justify-start items-center gap-1 flex">
                <div className="text-center text-[#175cd3] text-xs font-medium font-['Inter'] leading-[18px]">Engineering</div>
              </div>
            </div>
            <div className="justify-start items-start gap-2 inline-flex">
              ...
              <div className="pl-1 pr-1.5 py-0.5 bg-gray-50 rounded-md justify-start items-center gap-1 flex">
                <img 
                  ...
                  className="w-3 h-3"
                />
                <div className="text-center text-[#344054] text-xs font-medium font-['Inter'] leading-[18px]">Maxime Togbe</div>
              </div>
            </div>
          </div>
        </div>
