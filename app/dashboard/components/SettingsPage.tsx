...
export default function SettingsPage({ 
  ...
  const [currentBillingType, setCurrentBillingType] = useState<'monthly' | 'annual' | null>(null);

  // State for plan change confirmation modal
  const [showPlanChangeConfirmation, setShowPlanChangeConfirmation] = useState(false);
  const [pendingBillingPeriod, setPendingBillingPeriod] = useState<'monthly' | 'annual' | null>(null);

  const { data: subscriptionData, error: subscriptionError, mutate: mutateSubscription } = useSWR<{
    plan_type: string;
    status: string;
    ...
    `/api/subscription?workspaceId=${workspace?.id}`,
    fetcher,
    {
  ...
  const fetchSubscription = async () => {
    ...
    try {
      ...
      if (data.price_id) {
        ...
        if (data.price_id === STRIPE_PRICE_IDS.EARLY_ADOPTER.ANNUAL || 
            data.price_id === process.env.NEXT_PUBLIC_STRIPE_EARLY_ADOPTER_ANNUAL_PRICE_ID) {
      ...
      return data.plan_type === 'EARLY_ADOPTER' && 
  ...
  useEffect(() => {
    ...
    const showNotification = () => {
      ...
      const storedNotification = typeof window !== 'undefined' ? 
        localStorage.getItem('checkoutNotification') : null;
        ...
        try {
          ...
          if (notification.status === 'success') {
            ...
            toast.error('Checkout Failed', {
              description: notification.error || 'There was an issue processing your payment. Please try again.',
    ...
    if (checkoutStatus) {
      ...
      if (checkoutStatus === 'success' || checkoutStatus === 'failed') {
        ...
        localStorage.setItem('checkoutNotification', JSON.stringify(notificationData));
      }
    ...
    try {
      ...
      if (!response.ok) {
        toast.error('Failed to Downgrade', {
          description: data.details || data.error || 'An error occurred while downgrading your plan',
          duration: 5000,
        });
      ...
      toast.success('Successfully Downgraded', {
        description: 'Your subscription has been canceled. You will continue to have access to Early Adopter features until the end of your billing period.',
        duration: 7000,
      });
  ...
  const updateBillingPeriod = async (period: 'monthly' | 'annual') => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.details || data.error || 'Failed to update subscription');
      }
      ...
      toast.success(title, {
        description: data.details || `Your subscription has been updated to ${period} billing.`,
      });
  ...
  const PLAN_DATA = {
    ...
    earlyAdopter: {
      monthly: {
        ...
        priceId: process.env.NEXT_PUBLIC_STRIPE_EARLY_ADOPTER_MONTHLY_PRICE_ID || ''
      },
      annual: {
        price: "$12",
        period: "per user/month billed annually",
        priceId: process.env.NEXT_PUBLIC_STRIPE_EARLY_ADOPTER_ANNUAL_PRICE_ID || ''
      },
      name: "Early Adopter",
      features: [
        "All Free features +",
  ...
  const getPlanButtonText = (planType: 'free' | 'earlyAdopter', currentBillingType: 'monthly' | 'annual' | null, selectedBillingPeriod: 'monthly' | 'annual'): React.ReactNode => {
    ...
    if (planType === 'earlyAdopter') {
      ...
      if (currentBillingType === 'monthly' && selectedBillingPeriod === 'annual') {
        return (
      ...
      if (currentBillingType === 'annual' && selectedBillingPeriod === 'monthly') {
  ...
  const handlePlanButtonClick = async (planType: 'free' | 'earlyAdopter') => {
    ...
    if (currentPlan === 'earlyAdopter' && subscriptionStatus === 'ACTIVE' && 
  ...
  const confirmPlanChange = async () => {
    ...
    try {
      ...
      const toastTitle = isUpgrade ? 'Upgrading to Annual Billing' : 'Switching to Monthly Billing';
    ...
    <WorkspaceProvider value={{ workspace: workspace || null }}>
      ...
      >
          ...
          {activeTab === 'Plan' && (
            <div className="flex flex-col justify-center items-center">
              ...
              <div className="grid md:grid-cols-2 gap-8 w-fit">
                ...
                >
                  ...
                  <div className="mt-auto">
                    <ButtonNormal
                      ...
                      onClick={currentPlan !== 'free' ? handleDowngradeClick : undefined}
                    >
                ...
                >
                  {(currentPlan === 'earlyAdopter' && subscriptionStatus === 'ACTIVE') && (
                    <div 
                      style={{
                        backgroundColor: colors['bg-brand-primary'],
                        color: colors['text-brand-primary'],
                  ...
                  {(currentPlan === 'earlyAdopter' && subscriptionStatus === 'ACTIVE') ? (
                    ...
                    <ButtonNormal
                      ...
                      disabled={currentBillingType === billingPeriod || isUpdatingBillingPeriod}
                    >
                      {isUpdatingBillingPeriod ? 'Updating...' : getPlanButtonText('earlyAdopter', currentBillingType, billingPeriod)}
                    </ButtonNormal>
                  ) : (
                    ...
                    <CheckoutButton
                      ...
                      fullWidth
                    >
                      {isLoadingPlan ? 'Loading...' : getPlanButtonText('free', null, billingPeriod)}
                    </CheckoutButton>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-col gap-10 min-h-full">
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex flex-col gap-1 pt-1">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {currentPlan === 'earlyAdopter' ? 'Early Adopter' : 'Free Plan'}
                          </h3>
                          {currentPlan === 'earlyAdopter' && subscriptionStatus === 'ACTIVE' && (
                            <div 
                              style={{ 
                          ...
                          <div className="flex flex-col gap-1 items-end pt-1">
                            ...
                            <div className="flex items-center gap-2">
                              <span 
                                ...
                                className="text-lg font-semibold leading-7"
                              >
                                ${nextPaymentAmount ? (nextPaymentAmount / 100).toLocaleString() : '0'} on {
              ...
              <div className="px-8">
                <div className="flex flex-col gap-6">
                  ...
                  {/* Invoices Table */}
                  {Boolean(invoicesData?.invoices && invoicesData.invoices.length > 0) ? (
                    <div 
                      style={{ 
                        backgroundColor: colors['bg-secondary'],
                        borderColor: colors['border-secondary']
                    ...
                    >
                      ...
                      {invoicesData?.invoices
                        .filter(invoice => invoice.number && invoice.created && invoice.amount)
                        .map((invoice) => (
                        <div 
                          key={invoice.id}
                          style={{ 
        ...
        {showDowngradeConfirmation && (
          ...
          >
            <div className="space-y-4 py-2">
              ...
              <ul className="list-disc pl-5 space-y-2" style={{ color: colors['text-secondary'] }}>
              ...
              <p style={{ color: colors['text-secondary'] }} className="font-semibold">
        ...
        {showPlanChangeConfirmation && (
          <Modal
            title={pendingBillingPeriod === 'annual' ? 'Upgrade to Annual Billing' : 'Switch to Monthly Billing'}
            subtitle={pendingBillingPeriod === 'annual' 
            ...
            actions={
              <div className="flex justify-end gap-3 w-full">
                ...
                <ButtonNormal
                  ...
                  disabled={isUpdatingBillingPeriod}
                >
                  {isUpdatingBillingPeriod ? 'Confirming...' : (
            ...
            <div className="space-y-4 py-2">
              {pendingBillingPeriod === 'annual' ? (
                <>
                  ...
                  <ul className="list-disc pl-5 space-y-2" style={{ color: colors['text-secondary'] }}>
                    <li><strong>20% savings</strong> compared to monthly billing</li>
                    <li>Your subscription will be charged <strong>annually</strong> instead of monthly</li>
                    <li>Your new price will be <strong>{PLAN_DATA.earlyAdopter.annual.price} per user/month</strong>, billed annually</li>
                    <li>The change will take effect at the start of your next billing cycle</li>
                  </ul>
                  <p style={{ color: colors['text-secondary'] }} className="font-semibold">
                    You will continue to have access to all Early Adopter features during this transition.
                  </p>
                </>
              ) : (
                <>
                  ...
                  <ul className="list-disc pl-5 space-y-2" style={{ color: colors['text-secondary'] }}>
                    <li>Your subscription will be charged <strong>monthly</strong> instead of annually</li>
                    <li>Your new price will be <strong>{PLAN_DATA.earlyAdopter.monthly.price} per user/month</strong></li>
                    <li>You'll no longer receive the 20% annual discount</li>
                    <li>The change will take effect at the end of your current annual billing period</li>
                    <li>No refunds will be issued for the remaining time on your annual plan</li>
                  </ul>
                  <p style={{ color: colors['text-secondary'] }} className="font-semibold">
                    You will continue to have access to all Early Adopter features during this transition.
                  </p>
